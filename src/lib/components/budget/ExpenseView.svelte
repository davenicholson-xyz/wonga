...
<button
	class="flex items-center gap-2 w-full px-3 py-1.5 text-left transition-colors hover:bg-base-content/5 cursor-pointer"
	onclick={() => {
		modals.editExpense.show(expense.id, expense.description, expense.amount, expense.billspot);
	}}
>
	<div class="flex-1 min-w-0">
		<span class="text-xs">{expense.description}</span>
	</div>
	<div class="flex items-center gap-1.5 shrink-0">
		{#if expense.billspot}
			<div class="w-4 h-4 rounded bg-warning/15 flex items-center justify-center" title="Bills Pot">
		...
		{/if}
		<span class="text-xs font-semibold text-base-content/70">{formatCurrency(expense.amount)}</span>
	</div>
</button>
